...
//	private UserService userServ;
	...
	public List<Oglas> getAllRez(Long id) {
		List<Long> narId=narServ.getAllByRezNar(id).stream().map(x->x.getOglasId()).collect(Collectors.toList());	//idijevi oglasa  koje su vezane za rezervaciju preko narudzbenice
		List<Oglas> oglasi=new ArrayList<>();
		for(Long nId : narId) {
			Oglas o=oglasRep.findById(nId).orElse(null);
			if(o!=null)
			oglasi.add(o);
			//else throw new Exception("OglasService > getAllRez nije pronasao oglas za id="+nId);
		}
		return oglasi;
	}
