...
public class RezervacijaService {
	...
	public RezervacijaResponse postRezervacija(Long id, Long commonDataId, float ukupnaCena, boolean bundle, String statusRezervacije, String napomena) {
		Rezervacija body= new Rezervacija(id, commonDataId, ukupnaCena, bundle, statusRezervacije, napomena);
		List<Map> trans = (List<Map>) RESTClient.getClient().forService(Services.RESERVATION).withPath("/reservation/")
		...
		for (Map m : trans) {
			...
			t.setId(((Number) m.get("id")).longValue());
			t.setCommonDataId(((Number) m.get("commonDataId")).longValue());
			t.setBundle(Boolean.parseBoolean((String)m.get("bundle")));
			t.setUsername((String) m.get("username"));
			t.setStatusRezervacije((String) m.get("statusRezervacije"));
	...
	public NarudzbenicaResponse postNarudzbenica(Long id, Long commonDataId, Long agentId, Long userId, 
		...
		Narudzbenica body= new Narudzbenica(id, commonDataId, agentId, userId, 
	...
	public IzvestajResponse postIzvestaj(Long id, Long commonDataId, float predjenaKilometraza, String tekstIzvestaja, 
		...
		Izvestaj body=new Izvestaj(id, commonDataId, predjenaKilometraza, tekstIzvestaja, 
									narudzbenicaId, automobilId, rezervacijaId, 
									prekoracenaKilometraza, dodatniTroskovi);
		List<Map> trans = (List<Map>) RESTClient.getClient().forService(Services.RESERVATION).withPath("/report")
				.withMethod(HttpMethod.POST).withBody(body).send();
		List<Izvestaj> ret = new LinkedList<>();
		for (Map m : trans) {
			Izvestaj t = new Izvestaj();
