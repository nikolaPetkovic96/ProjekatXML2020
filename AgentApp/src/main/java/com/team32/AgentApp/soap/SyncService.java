...
//
//				if(userRep.findById(u.getId())!=null) {	//vec postoji korisnik sa tim id, 
//					User uBazi=userRep.findById(u.getId()).orElse(null);
//					//izvrsiti provere sta se sve moze menjati
//					System.out.println("CMDid : " +uBazi.getCommonDataId() );
//					//update commonData
//					CommonData cmdUBazi=commonDataRep.findById(uBazi.getCommonDataId()).orElse(null);
//					cmdUBazi.setDatumIzmene(LocalDateTime.now());
//					cmdUBazi=commonDataRep.save(cmdUBazi);
//					uBazi.setCommonDataId(cmdUBazi.getId());
//					//
//					User updated=uBazi;
//					userRep.deleteById(uBazi.getId());
//					uBazi=userRep.save(updated);
//					
//				}else {
					CommonData commonData = new CommonData(u.getId());	//kreira id sa trenutnim vremenom i prosledjenim userId
					commonData=commonDataRep.saveAndFlush(commonData);
					u.setCommonDataId(commonData.getId());
					u=userRep.saveAndFlush(u);
